...
def try_generate_content(api_key, image):
    ...
    try:
        # 콘텐츠 생성 시도
        response = model.generate_content(["이 사진은 초등학생이 직접 그린 미술 작품입니다. 그림 이름을 추천해주고, 그린 작품을 자세히 설명해주세요. 장점을 여러 개 말해주고, 보완할 점 1가지를 학생이 상처받지 않도록 온화한 화법으로 말해주세요.", image])
        response.resolve()
        return response
...
# 핸드폰 사진 업로드 기능 추가
uploaded_file = st.file_uploader("핸드폰으로 학생이 그린 미술 작품을 가로로 찍어주세요.", type=["jpg", "jpeg", "png"])
