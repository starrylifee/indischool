...
try:
    ...
    st.error("secrets.toml 파일을 찾을 수 없습니다. 파일 경로를 확인해주세요.")
    secrets = {}
...
example_text = """민수는 체육대회에서 친구들과 함께 열심히 뛰면서 서로 응원하고 도와주었어요.
지수는 우리 반 친구들과 함께 과학 프로젝트를 준비하며 많은 시간을 함께 보냈고, 결국 좋은 성과를 거두었어요.
소라는 새 학기에 전학 온 친구를 반 친구들과 함께 도와주고 학교 생활에 적응할 수 있도록 도왔어요."""

input_text = st.text_area("친구의 좋은점이나 함께 놀았던 추억을 적어보세요.:", example_text)

if st.button("상장명과 문구 생성"):
    if not input_text.strip():
        st.error("입력 필드를 채워주세요.")
    else:
        prompt_parts = [
            ...
            "input: 라운이는 급식실에서 내가 먹고 싶은 음식을 줬어요",
            "output 2: 급식실 천사상: 라운이는 급식실에서 친구가 먹고 싶은 음식을 챙겨주는 따뜻한 마음을 가진 친구입니다! 친구를 배려하는 마음씨에 감동했어요!",
            f"input: {input_text}",
            "output 2: ",
        ]
        
        if gemini_api_key3 or gemini_api_key4:
            # 첫 번째 API 키로 시도
